...
- Optionally deletes rows with negative sentiment from the table if enabled.
...
- `ENABLE_DELETE_NEGATIVE_ROW`: Set to "true" to delete rows with negative sentiment from the table.
...
4. If enabled, rows with negative sentiment are deleted from the table.
...
2. Set the necessary environment variables. For example:
   ...
   export ENABLE_DELETE_NEGATIVE_ROW="true"  # Optional: set to "true" to delete negative rows
   ```
